...
ground_pos_x = ground_pos[1] or ground_pos_x;
ground_pos_y = ground_pos[2] or ground_pos_y;
ground_pos_z = ground_pos[3] or ground_pos_z;
if type(PI.ground_pos) == "table" then
	ground_pos_x = tonumber(PI.ground_pos[1]) or ground_pos_x;
	ground_pos_y = tonumber(PI.ground_pos[2]) or ground_pos_y;
	ground_pos_z = tonumber(PI.ground_pos[3]) or ground_pos_z;
end
...
camera_pos_x = camera_pos[1] or camera_pos_x;
camera_pos_y = camera_pos[2] or camera_pos_y;
if type(PI.camera_pos) == "table" then
	camera_pos_x = tonumber(PI.camera_pos[1]) or camera_pos_x;
	camera_pos_y = tonumber(PI.camera_pos[2]) or camera_pos_y;
end
...
pos_x = pos[1] or pos_x;
pos_y = pos[2] or pos_y;
if type(PI.pos) == "table" then
	pos_x = tonumber(PI.pos[1]) or pos_x;
	pos_y = tonumber(PI.pos[2]) or pos_y;
end
...
max_w = tonumber(PI.max_w) or max_w;
max_h = tonumber(PI.max_h) or max_h;
...
if obj.getinfo("filter") then
	...
	local W, H = obj.getinfo("image_max");
...
local l_dx, l_dy, l_dz, l_ddx0, l_ddy0, l_ddz0 do -- direction of the light and range of the conic blur.
	...
	g_nw = -(g_nx * ground_pos_x + g_ny * ground_pos_y + g_nz * ground_pos_z);
...
---@diagnostic disable-next-line: missing-return-value
	...
	local p = 1 / Z;
	return p * x + (1 - p) * camera_pos_x, p * y + (1 - p) * camera_pos_y;
end
...
local function to_back_camera(dx, dy, dz, x, y, z)
	...
	x, y = x - camera_pos_x, y - camera_pos_y;
...
local function vanishing_pts(dx, dy, dz)
	if camera_fov <= 0 or dz == 0 then
		...
		local px, py = p * dx + camera_pos_x, p * dy + camera_pos_y;
		return
...
if len > 0 then
	...
	obj.effect("斜めクリッピング",
		"中心X", ground_pos_x - (len - L / 2) * sin_rot,
		"中心Y", ground_pos_y + (len - L / 2) * cos_rot,
		"角度", rot, "ぼかし", L);
	obj.effect("斜めクリッピング",
		"中心X", ground_pos_x + (len - L / 2) * sin_rot,
		"中心Y", ground_pos_y - (len - L / 2) * cos_rot,
...
local w1, h1, w2, h2, x2, y2, ray_range = obj.w, obj.h, nil, nil, nil, nil, 0 do
	...
	local corners do
		...
		if len > 0 then
			...
			if cos_rot ~= 0 then
				...
				local Y1, Y2, Y3, Y4 =
					(ground_pos_y + ly) - tan_rot * (ground_pos_x + lx - R),
					(ground_pos_y + ly) - tan_rot * (ground_pos_x + lx - L),
					(ground_pos_y - ly) - tan_rot * (ground_pos_x - lx - R),
					(ground_pos_y - ly) - tan_rot * (ground_pos_x - lx - L);
			...
			if sin_rot ~= 0 then
				...
				local X1, X2, X3, X4 =
					(ground_pos_x + lx) - cot_rot * (ground_pos_y + ly - B),
					(ground_pos_x + lx) - cot_rot * (ground_pos_y + ly - T),
					(ground_pos_x - lx) - cot_rot * (ground_pos_y - ly - B),
					(ground_pos_x - lx) - cot_rot * (ground_pos_y - ly - T);
	...
	-- reposition.
	L, T, R, B = L + pos_x, T + pos_y, R + pos_x, B + pos_y;
...
if alpha > 0 then
	local ofs_x2, ofs_y2 = w2 / 2 + x2 + pos_x, h2 / 2 + y2 + pos_y;
	...
	{
		w1, h1; w1 / 2 - ofs_x2, h1 / 2 - ofs_y2;
		camera_pos_x + ofs_x2, camera_pos_y + ofs_y2, camera_fov > 0 and -1024 / camera_fov or 0;
		alpha1;
